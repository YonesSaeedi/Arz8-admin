...
export default function useUsersList(isCrypto=true) {
    ...
    const tableColumns = [
        ...
        {key: 'amount_sell', label: 'مقدار فروش به ما'},
        {key: 'avg_price_sell', label: 'میانگین نرخ فروش به ما', sortable: true},
        {key: 'amount_buy', label: 'مقدار خرید از ما', sortable: true},
        {key: 'avg_price_buy', label: 'میانگین نرخ خرید از ما', sortable: true},
        {key: 'benefit', label: 'سود/زیان', sortable: true},
        {key: 'custs', label: 'هزینه ها', sortable: true},
        {key: 'net', label: 'خالص', sortable: true},
        {key: 'balance', label: 'موجودی',sortable: true},
        {key: 'orders', label: 'سفاراشات',sortable: false},
    ];
    ...
    const fetchOrders = (ctx, callback) => {
        sumBenefit.value = null;
        sumCust.value = null;
        axios.post('audit/daily', {
            ...
            .then(response => {
                const {list, total,sum_benefit,sum_cust} = response.data

                callback(list)
                totalRows.value = total
                sumBenefit.value = sum_benefit
