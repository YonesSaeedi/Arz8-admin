    ...
    <div id="level3-form">
        ...
        >
            ...
            <b-col md="8" class="mx-auto">
                <p>
                    اطلاعات سطح سه در این بخش قابل ثبت و ویرایش میباشد! اطلاعات این سطح باید به تایید شما برسد و
                    اگر بعد از تایید سطح آن کمتر از چهار باشد، به سطه چهار ارتقاء میباید.
                </p>

                <b-alert variant="warning" show v-if="user.auth_img && user.auth_img.status === 'pending'">
                    <div class="alert-body">
                        <span><strong>توجه!</strong> اطلاعات این بخش در درست بررسی قرار دارد و لطفا نسبت به رد یا تایید آن اقدام کنید.</span>
                    </div>
                </b-alert>
                <b-alert variant="danger" show v-else-if="user.auth_img && user.auth_img.status === 'reject'">
                    <div class="alert-body">
                        <span><strong>توجه!</strong>
                            اطلاعات این بخش به دلیل
                            "{{user.auth_img['reason_'+(user.settings.localization?user.settings.localization:'fa')]}}"
                            تایید نشده است.</span>
                    </div>
                </b-alert>
                <b-alert variant="success" show v-else-if="user.auth_img && user.auth_img.status === 'confirm'">
                    <div class="alert-body">
                ...
                <b-row>
                    <b-col md="12" class=" mx-auto">
                        <file-pond
                            ...
                            :files="user.auth_img && user.auth_img.file ? myFiles: null"
                            :processfile="null"
                        />
                    </b-col>
                    <b-col md="12">
                        <b-button v-if="user.auth_img && user.auth_img.file"
                                  size="sm" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mr-2"
                                  v-b-modal.modal-level3-file>
                            <feather-icon icon="DownloadIcon" class="mr-50"/>
                            مشاهده تصویر
                        </b-button>

                        <b-button size="sm" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mr-md-2 mt-md-0 mt-25"
                                  v-b-modal.modal-level3-list-file v-if="user.auth_img && user.auth_img.file">
                            <feather-icon icon="ListIcon" class="mr-50"/>
                            لیست تصاویر آپلودی
                        </b-button>

                        <b-button v-if="user.auth_img && (user.auth_img.status === 'reject' || user.auth_img.status === 'pending')"
                        ...
                        <b-button v-if="user.auth_img && (user.auth_img.status === 'confirm' || user.auth_img.status === 'pending')"
        ...
        <!-- basic modal -->
        <b-modal id="modal-level3-file" scrollable size="lg" title="تصویر" ok-only ok-title="باشه">
            <a :href="baseAdminURL+'image/'+ user.auth_img.file_hash +'/image.jpg'" target="_blank" v-if="user && user.auth_img && user.auth_img.file_hash">
                <img :src="baseAdminURL+'image/'+ user.auth_img.file_hash +'/image.jpg'"  width="100%" class="box-shadow-1 border">
            </a>
            <!--<img :src="srcFile" width="100%" class="box-shadow-1 border">-->
        </b-modal>

        <!-- basic modal -->
        <b-modal id="modal-level3-list-file" scrollable size="xl" title="لیست تصاویر آپلود شده" ok-only ok-title="باشه"
                 v-if="user.auth_img && user.auth_img.file">
            <div class="table-responsive mb-1">
                <table class="table b-table table-striped">
                    ...
                    <tbody>
                    <tr v-for="(item, indextr) in user.auth_img.file">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                myFiles: [
                    {
                        source:
                            "users/edit/level3/"+ this.user.id +"/file",
                        options: {
                            type: "local"
                        }
                    }
        ...
        methods:{
            onSubmit(){
                this.$refs.refFormLevel4Observer.validate().then(success => {
                    if (success) {
                        ...
                        if(this.$refs.pond.getFiles()[0] && this.$refs.pond.getFiles()[0].file && this.$refs.pond.getFiles()[0].file.name !=='file')
                            ...
                            this.$swal({icon: 'warning',title: 'ناموفق!',text: 'برای تغییر نیاز است تصویر جدید بارکذاری کنید.',confirmButtonText: 'باشه'});
                            return;
                        }

                        this.isLoading = true;
                        axiosIns.post('/users/edit/level3/'+this.user.id,bodyFormData,{
            ...
            confirm(){
                this.$swal({
                    ...
                    preConfirm: () => {
                        return  axiosIns.put('/users/edit/level3/'+this.user.id +'/status',{status:'confirm'})
                            .then(response => {
                                return response;
                            })
                            .catch(() => {
            ...
            reject(){
                this.$swal({
                    ...
                    buttonsStyling: false,
                    preConfirm: (value) => {
                        return  axiosIns.put('/users/edit/level3/'+this.user.id +'/status',{status:'reject',reason:value})
                            .then(response => {
                                return response;
                            })
                            .catch(() => {
            ...
            setDefault(){
                ...
                //if(this.user.auth_img && this.user.auth_img.file_hash)
                    //this.getFile(this.user.auth_img.file_hash,'srcFile')
            }
        },
        watch:{
            user(){
