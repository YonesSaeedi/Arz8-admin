    ...
    <div id="level2-form">
        ...
        >
            ...
            <b-col md="8" class="mx-auto">
                ...
                <b-alert variant="warning" show v-if="user.identification_img && user.identification_img.status === 'pending'">
                    <div class="alert-body">
                        <span><strong>توجه!</strong> اطلاعات این بخش در درست بررسی قرار دارد و لطفا نسبت به رد یا تایید آن اقدام کنید.</span>
                    </div>
                </b-alert>
                <b-alert variant="danger" show v-else-if="user.identification_img && user.identification_img.status === 'reject'">
                    <div class="alert-body">
                        <span><strong>توجه!</strong>
                            اطلاعات این بخش به دلیل
                            "{{user.identification_img['reason_'+(user.settings.localization?user.settings.localization:'fa')]}}"
                            تایید نشده است.</span>
                    </div>
                </b-alert>
                <b-alert variant="success" show v-else-if="user.identification_img && user.identification_img.status === 'confirm'">
                    <div class="alert-body">
                ...
                <b-row>
                    <b-col md="12" class=" mx-auto">
                        <file-pond
                            ...
                            :files="user.identification_img && user.identification_img.file ? myFiles: null"
                            :processfile="null"
                        />
                    </b-col>
                    <b-col md="12">
                        <b-button v-if="user.identification_img && user.identification_img.file"
                        ...
                        <b-button v-if="user.identification_img.status === 'reject' || user.identification_img.status === 'pending'"
                        ...
                        <b-button v-if="user.identification_img.status === 'confirm' || user.identification_img.status === 'pending'"
        ...
        <b-modal id="modal-level2-file" scrollable size="lg" title="تصویر" ok-only ok-title="باشه">
            <a :href="baseAdminURL+'image/'+ user.identification_img.file_hash +'/image.jpg'" target="_blank">
                <img :src="baseAdminURL+'image/'+ user.identification_img.file_hash +'/image.jpg'"  width="100%" class="box-shadow-1 border">
            </a>
        ...
        <b-modal id="modal-level2-list-file" scrollable size="xl" title="لیست تصاویر آپلود شده" ok-only ok-title="باشه"
                    v-if="user.identification_img && user.identification_img.file">
            <div class="table-responsive mb-1">
                <table class="table b-table table-striped">
                    ...
                    <tbody>
                    <tr v-for="(item, indextr) in user.identification_img.file">
        ...
        data() {
            return {
                ...
                myFiles: [
                    {
                        source:
                            "users/edit/level2/"+ this.user.id +"/file",
                        options: {
                            type: "local"
                        }
                    }
        ...
        methods:{
            onSubmit(){
                this.$refs.refFormLevel3Observer.validate().then(success => {
                    if (success) {
                        ...
                        axiosIns.post('/users/edit/level2/'+this.user.id,bodyFormData,{
            ...
            confirm(){
                this.$swal({
                    ...
                    preConfirm: () => {
                        return  axiosIns.put('/users/edit/level2/'+this.user.id +'/status',{status:'confirm'})
                            .then(response => {
                                return response;
                            })
                            .catch(() => {
            ...
            reject(){
                this.$swal({
                    ...
                    buttonsStyling: false,
                    preConfirm: (value) => {
                        return  axiosIns.put('/users/edit/level2/'+this.user.id +'/status',{status:'reject',reason:value})
                            .then(response => {
                                return response;
                            })
                            .catch(() => {
            ...
            setDefault(){
                ...
                if(this.user.identification_img==null){
                    this.user.identification_img = this.user.identification_img? this.user.identification_img:[];
                }

                //if(this.user.identification_img && this.user.identification_img.file_hash)
                    //this.getFile(this.user.identification_img.file_hash,'srcFile')
            },
        },
        watch:{
            user(){
