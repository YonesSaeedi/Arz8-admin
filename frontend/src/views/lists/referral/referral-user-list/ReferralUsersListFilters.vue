...
<script>
    ...
    export default {
        ...
        watch:{
            ...
            countStart(val){
                this.countStart = this.amountLocalFloat(val,0)
                this.$emit('update:countStartFilter', val.replace(/[^\d.-]/g, ''))
            },
            countStop(val){
                this.countStop = this.amountLocalFloat(val,0)
                this.$emit('update:countStopFilter', val.replace(/[^\d.-]/g, ''))
            },
            isLoading(val){
                if(!val)
